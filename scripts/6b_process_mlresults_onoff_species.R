...
path_true <- 'rural_urban_dich_species/output_XGB_class_rural_urban_dich_species_2024_02_28__13-15-00'
plot_feature_importance_class(path_true, 20)
plot_feature_importance_color_microbiome(path_true, 20)

path_true <- 'urban_ams_dich_species/output_XGB_class_urban_ams_dich_species_2024_02_28__13-13-39'
plot_feature_importance_class(path_true, 20)
plot_feature_importance_color_microbiome(path_true, 20)

## Data
mb <- readRDS("data/phyloseq_sampledata.RDS")
tax <- readRDS("data/taxtable_species.RDS")
tax <- as.data.frame(tax)
tax$ASV <- rownames(tax)
tax$Tax <- str_c(tax$Genus, " ", tax$Species)
feat1 <- rio::import('rural_urban_dich_species/output_XGB_class_rural_urban_dich_species_2024_02_28__13-15-00/feature_importance.txt') %>% slice(1:10)
feat2 <- rio::import('urban_ams_dich_species/output_XGB_class_urban_ams_dich_species_2024_02_28__13-13-39/feature_importance.txt') %>% slice(1:10)
...
ggsave(pl1, filename = "results/vanishingmicrobes_species_1.pdf", width = 10, height = 5, )
...
(pl2 <- ggplot(data = dfsum, aes(x = ASV, group = Site, fill = Site, y = percentage)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1)))
ggsave(pl2, filename = "results/vanishingmicrobes_species_2.pdf", width = 10, height = 5)

ggarrange(pl1, pl2, nrow = 2, 
          common.legend = TRUE, 
          legend = "bottom",
          labels = c("A", "B"))
ggsave("results/vanishingmicrobes_species.pdf", width = 10, height = 10)
