...
#### Bray-Curtis distance: male-female ####
print('Bray-Curtis distance small dataset')
amsids <- df_new %>% filter(Site == "Amsterdam" & !is.na(TimeLoc)) %>% select(ID)
tabs <- tab %>% filter(rownames(.) %in% amsids$ID)
bray <- vegan::vegdist(tabs, method = 'bray')
pcoord <- ape::pcoa(bray, correction = "cailliez")
str(pcoord$values)
expl_variance_bray <- pcoord$values$Rel_corr_eig * 100
head(expl_variance_bray)
...
set.seed(1234)
dfanova <- df_new %>%
    filter(!is.na(TimeLoc)) %>% 
    slice(match(rownames(tabs), ID)) # distance matrix and metadata must have the same sample order
all(dfanova$ID == rownames(tabs)) # TRUE
dim(dfanova)
res1 <- adonis2(bray ~ TimeLoc, data = dfanova) # PERMANOVA
print(res1)

print('plotting..')
(braycurt <- dbray %>% 
        ggplot(aes(Axis.1, Axis.2)) +
        geom_point(aes(color = TimeLoc), size = 2) +
        xlab(paste0('PCo1 (', round(expl_variance_bray[1], digits = 1),'%)')) +
        ylab(paste0('PCo2 (', round(expl_variance_bray[2], digits = 1),'%)')) +
        scale_color_gradient(low = pal_cosmic()(4)[2], high = pal_cosmic()(5)[4]) +
        theme_Publication() +
        labs(color = "", fill = "", title = "Time in Amsterdam") +
        theme(legend.position = "top", legend.key.height = unit(0.5, 'cm'), 
              legend.key.width = unit(1, 'cm')) +
        annotate("text", x= Inf, y = Inf, hjust = 1, vjust = 1,
                 label = str_c("PERMANOVA: p = ", res1$`Pr(>F)`, ", r2 = ", format(round(res1$R2[1],3), nsmall = 3))))
ggsave(braycurt, filename = "results/ordination/PCoA_timeAms.pdf", device = "pdf", width = 5, height = 5)
ggsave(braycurt, filename = "results/ordination/PCoA_timeAms.svg", device = "svg", width = 5, height = 5)
...
phylach <- subset_taxa(phydata, Family == "Lachnospiraceae")
phylach <- subset_samples(phylach, !is.na(SodiumInt))
wunifrac <- UniFrac(phylach, normalized = T, weighted = T, parallel = T)
pcoord <- ape::pcoa(wunifrac, correction = "cailliez")
expl_var_wu <- pcoord$values$Rel_corr_eig * 100
head(expl_var_wu)
dfpc <- pcoord$vectors[, c('Axis.1', 'Axis.2')] # get PCoA coordinates
...
dfpc$ID <- rownames(dfpc)
dfpc <- left_join(dfpc, df_new, by = 'ID') %>%  # add metadata / covariates
    filter(!is.na(SodiumInt))
print('PERMANOVA..')
res2 <- adonis2(wunifrac ~ SodiumInt, data = dfpc) # PERMANOVA
print(res2)

(unifracpl <- dfpc %>% 
        ggplot(aes(Axis.1, Axis.2)) +
        geom_point(aes(color = SodiumInt), size = 1, alpha = 0.7) +
        theme_Publication() +
        # stat_ellipse(geom = "polygon", aes(color = SaltBin, fill = SaltBin), type = "norm", alpha = 0.1,
        #              linewidth = 1.0) +
        # scale_color_manual(values = pal_cosmic()(4)[c(2:4)]) +
        # scale_fill_manual(values = pal_cosmic()(4)[c(2:4)], guide = "none") +
        theme(legend.position = "top") +
