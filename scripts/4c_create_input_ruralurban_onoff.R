...
# relative abundances microbes
relab <- otu[,tk]
relab <- as.data.frame(relab) %>% mutate(across(everything(.), ~ (.x / 15000) *100))
relab$ID <- rownames(relab)
rs <- relab %>% pivot_longer(., cols = 1:ncol(.)-1, names_to = "ASV", values_to = "abundance") %>% 
    group_by(ASV) %>% 
    summarise(mean_ab = mean(abundance)) 
rs %>% arrange(-mean_ab, ASV) %>% 
    ggplot(.) +
    geom_histogram(aes(x = mean_ab)) +
    scale_x_log10() +
    theme_Publication() +
    labs(x = "Abundance in %")
...
# make input data
path <- 'rural_urban_dich'
dir.create(path)
dir.create("rural_urban_dich/input_data")
...
# make input data
path <- 'rural_urban_dich_genus'
dir.create(path)
dir.create("rural_urban_dich_genus/input_data")
write_data(mbdf, file.path(path, 'input_data'))
y <- as.data.frame(df$Site)
y
write_y(y, name_y = 'y_binary.txt', file.path(path, 'input_data'))
...
# make input data
path <- 'rural_urban_dich_species'
dir.create(path)
dir.create("rural_urban_dich_species/input_data")
write_data(mbdf, file.path(path, 'input_data'))
y <- as.data.frame(df$Site)
y
write_y(y, name_y = 'y_binary.txt', file.path(path, 'input_data'))
