...
res_diet3 <- res_diet2 %>% 
    ...
    mutate(model = factor(model, levels = c("0"), 
                          labels = c("Unadjusted")),
           Diet = factor(Diet, levels = colnames(df)[31:37]),
           Diet = fct_rev(Diet))

(pldiet <- ggplot(res_diet3, aes(x=Diet, y=est)) +
        ...
        scale_color_cosmic() +
        coord_flip())
ggsave("results/glm_clr/lm_diet.pdf", width = 4, height = 3)
ggsave("results/glm_clr/lm_diet.svg", width = 4, height = 3)
